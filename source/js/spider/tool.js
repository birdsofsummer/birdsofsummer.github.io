...
function showLoginWaitCurr(top,left){
	var page = $('body');
	if(isNull(page.find('#loginWait')) || page.find('#loginWait').length <= 0){
		var loadingHtml =	'\
...
*       默认情况下，changePage() 会忽略跳转到已活动的页面的请求。如果把这项设为true,会使之执行。
*       开发者应该注意有些页面的转场会假定一个跳转页面的请求中来自的页面和目标的页面是不同的，所以不会有转场动画。
...
function changePage(url) {
	showLoginWaitCurr();
	$.mobile.changePage(url,{transition:'none',showLoadMsg:false,allowSamePageTransition:true});
	//hideLoginWaitCurr();
}
 ...
 *
 * @Parameters
 *     addClick($('#targetbutton'), function(){
 *          alert('提交');
 *     })
 *
 */
function addClick(target,func){
	target.off('click').on("click", func);
}
/*
 * 公用添加事件
 *
 * @Parameters
 *	fun:要执行的事件
 *  name:事件名称   click、change等  默认click
 *
 */
$.fn.addEventFun = function(fun,name){
	name = isNull(name)?'click':name;
	return $(this).off(name).on(name,fun);
}
/**
 * 判断当前日期是否为月末最后一天
 */
function isLastDay(){
	//当年年月日及完整时间戳
	var day = '',month = '',year = '',serviceDate  = '',getDate = '';
	//获取相应头
	...
	xhr.onreadystatechange=function(){
		if( xhr.readyState == 4 && xhr.status == 200 ){
			...
			if(toDate.getDate() == day){
				$('#end_month').show();//显示
				addClick($("#end_month_close"),function(){
					$('#end_month').hide();//关闭
				});
				addClick($("#end_month_sure"),function(){
					$('#end_month').hide();//关闭
				});
			//不是月末
			}else{
...
/**
 * 根据服务器时间返回当前日期对象
 * func	回调函数 date_time getDate serviceDate
 * t多次请求更新时间
 */
function getServerDate(func,t){
	//当年年月日及完整时间戳
	var serviceDate  = '',getDate = '',date_time={},hrefStr='';
	//获取相应头
	var xhr = new XMLHttpRequest();
...
var Carousel = function(opt){
	...
	this.getHtml = function(){
		...
		$.each(this.setting.data,function(i,n){
			...
			point.push($('<li>',{class:i == 0?'curPoint':''}));
		});
		html = [];
		html.push($('<ul>',{
			html:lis,
	...
	this.swipe = function(action){
		...
		var div = $('#'+this.setting.dId);
		var ul = div.children('ul:first');
		if(!ul.is(':animated')){
			var wid = div.width();
			var _this = this;
			
			var max = this.setting.data.length;
			var index = ul.children('li').index();
			if(action=='left'){
				index++;
			...
			var nextsrc = ul.children('li').eq(index).find("img");
			if(!nextsrc.attr("src") || nextsrc.attr("src") == ""){
				nextsrc.attr({ src: nextsrc.attr("img_src") , img_src: ""});
				ul.children('ul').eq(index).fadeIn(300);
			}
			if(action == 'left'){
				ul.animate({left:'-='+wid+'px'},500,'linear',function(){
					$(this).css('left','0px').children(':first').appendTo($(this));
					if(_this.setting.stepPoint && _this.setting.pointMove){
						_this.culPoint(action,ul.next());
					}
					_this.setting.stepFun(action);
				});
			}else{
	...
	//计算亮点位置
	this.culPoint=function(action,ul){
		var max = this.setting.data.length;
		var index = ul.children('li.curPoint').index();
		if(action=='left'){
			index++;
		}else if(action=='right'){
			index--;
		...
		ul.children('li:eq('+index+')').addClass('curPoint').siblings().removeClass('curPoint');
	}
	//加载图片方式
	this.imgLoad = [
		function(ul){
			ul.find('img').width('100%');
		},
		function(ul){
			var aImgInfos = [];//图片集合
			var oImgInfo = {};//单个图片
			...
			ul.find('img').on('load',function(){
				...
				aImgInfos.push(oImgInfo);
				imgCount++;
				if(imgCount == this.setting.data.length){
					aImgInfos.sort(function(a,b){return a.iHeight>b.iHeight?1:-1});	//排序，从小到大
